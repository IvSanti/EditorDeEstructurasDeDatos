 ...
 *
 */
public class LogNodoGrafiColas {
	...
	public void verificarNodosInterceptados(Cola<NodoGrafico> cola) {
		
		for (int i = 0; i < cola.size(); i++) {
			for (int j = 0; j < cola.size(); j++) {

				if (i != j && cola.get(i).getColision().intersects(cola.get(j).getColision())) {
					Point nuevo = new Point(cola.get(j).getPunto().x + Constantes.RADIO_NODO * 2, cola.get(j).getPunto().y);
					cola.get(j).setPunto(nuevo);
				}
			//	verificarUbicacionNodo(lista.get(i));
			}
		}
	 ...
	 * @param pila
	 */
	public NodoGrafico verificarInterceptoMouse(Point p, Cola<NodoGrafico> cola) {

		Rectangle mouse = new Rectangle(p.x, p.y, REC, REC);

		for (int i = 0; i < cola.size(); i++) {
			if (mouse.intersects(cola.get(i).getColision())) {
				cola.get(i).setSeleccionado(true);
				return cola.get(i);
			}
		}
		return null;
	 ...
	 * @param cola
	 */
	public boolean identificarEnlaces(Cola<NodoGrafico> cola, ArrayList<NodoGrafico> nodoSeleccionado,
		...
		if(parejaNodos.isEmpty()){
			...
			if (cola.size()-1 == nodoSeleccionado.get(1).getIndice() && nodoSeleccionado.get(0).getIndice() == (parejaNodos.get(parejaNodos.size()-1)).getNodo2().getIndice()) {
